...
- [ ] 새 디렉터리 생성: `src/ui/{primitives,features}`, `src/state/`, 빈 배럴 `index.ts`
- [ ] `src/ui/README.md` 에 컴포넌트 규약(네이밍/props 패턴) 1페이지 명시
- [ ] 프리뷰 라우트 스캐폴드(`src/ui/preview/`) — 프리미티브 갤러리(개발 중 눈으로 확인용)
