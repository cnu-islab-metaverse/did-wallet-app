...
import { DEFAULT_NETWORKS } from '../types/network'
...
export async function initDevWallet() {
  ...
  try {
    ...
    // DEV_CONFIG.wallet 은 mnemonic·privateKey 키를 모두 가지므로 'in' 검사는 항상 참이다.
    // 키 존재가 아니라 실제 값으로 분기해야 폴백이 의도대로 동작한다.
    const devMnemonic = DEV_CONFIG.wallet.mnemonic?.trim().toLowerCase()
    const devPrivateKey = DEV_CONFIG.wallet.privateKey
    if (devMnemonic) {
      // Use Mnemonic to get seed, then create HDNodeWallet from seed to specify exact path
      const mnemonicObj = Mnemonic.fromPhrase(devMnemonic)
      const seed = mnemonicObj.computeSeed()
      const rootNode = HDNodeWallet.fromSeed(seed)
      ...
      const cleanKey = devPrivateKey.startsWith('0x')
        ? devPrivateKey
        : `0x${devPrivateKey}`
