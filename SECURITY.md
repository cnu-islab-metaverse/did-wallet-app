...
v2 를 배포한 뒤 발급기관 키 분리를 함께 반영해 **v3** 로 다시 올렸다(같은 날, 같은 이유 — 회로 변경).

```
Sepolia v3 (2026-09-06) — 최종
...
폐기됨
  v1 0xF66B3b93…  회로 취약점
  v2 0x11AbB46d…  단일 발급기관 키
...
### 발급기관 권한 범위 — 같은 날 함께 해소

취약점 조사 중에 **6개 발급기관이 단일 키를 공유**하는 것도 드러났다. 회로가
"이 키로 서명된 어떤 증명서"까지만 구분하므로 발급기관별 권한 범위를 강제하지 못했다 —
대학이 발급한 증명서로 거주를 증명할 수 있었다. 회로를 어차피 재빌드하는 김에 함께 고쳤다.

기관마다 별개의 키를 두고(`witness.mjs` 의 `ISSUERS`), 회로가 **시나리오별 발급기관
화이트리스트**를 one-hot 으로 검사한다(`_registry.circom`). 공개키 한 쌍(Ax,Ay)이 목록의
같은 인덱스에서 둘 다 일치해야 한다.
...
```
행정안전부 주민등록증으로 청년패스            → 성공   (허용 기관)
충남대가 거주지를 담아 발급 → 청년패스        → 거부 ✓ (권한 범위 강제)
경찰청 운전면허증으로 청년패스                → 성공   (허용 기관)
```

**남은 한계**: 구분은 **기관 단위**까지다. 증명서 *종류* 는 여전히 서명에 들어 있지 않으므로,
한 기관이 소관 밖 클레임을 담아 발급하면 그 기관이 허용된 시나리오 안에서는 통과한다
(행정안전부가 `university` 를 담는 등). `CLAIM_KEY` 에 `credentialType` 을 추가하는 것이 다음 단계다.
...
| 재배포 | v2(건전성 수정) → v3(발급기관별 키 분리) + 주소 전파(지갑·검증자·메타버스 씬) |
